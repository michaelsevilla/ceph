...
int JournalTool::main_event(std::vector<const char*> &argv)
{
  ...
  if (command == "get") {
    ...
    std::clock_t end;
    int nevents = 0;
    ndumps = 1; // we want the base image journal list to be 0 so we make sure to that we don't break out of this loop
    for (int i = 0; i < nfiles; i++) {
      ...
      dout(10) << "appending pos=" << 892 + 892 + 892 + 892*nevents + max << " nevents=" << nevents << " ndumps=" << ndumps << dendl;
      le->metablob.append_open(fname, ino, decoupled_ino, decoupled_eu->metablob.get_lump_map());
      js.events[892 + 892 + 892 + 892*nevents + max] = JournalScanner::EventRecord(le, 892, ndumps);
      nevents++;
      ...
      if (namespace_sync > 0 && time_elapsed >= namespace_sync) {
        ...
        if (pid > 0) { // child
          ...
          dout(0) << "write to local disk for ndumps=" << ndumps << dendl;
          for (JournalScanner::EventMap::const_iterator i = js.events.begin(); i != js.events.end(); ++i) {
            dout(10) << "event version=" << i->second.version << " type=" << i->second.log_event->get_type() << dendl;
            if (i->second.version != ndumps && i->second.version != 0)
              break;
          ...
          events_b1.write_file((file + to_string(ndumps)).c_str());
          kill(pid , SIGKILL);
        }
        nevents = 0; // Start overwriting
        ndumps++;
        start = std::clock();
      }
    }
  ...
  if (persist) {
    bufferlist events_bl;
    dout(0) << "write to local disk (last) ndumps=" << ndumps << dendl;
    for (JournalScanner::EventMap::const_iterator i = js.events.begin(); i != js.events.end(); ++i) {
      dout(10) << "event version=" << i->second.version << " type=" << i->second.log_event->get_type() << dendl;
      if (i->second.version != ndumps && i->second.version != 0)
        break;
